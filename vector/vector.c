 ...
 * OR whe the user calls on vector_reserve().
...
vector *vector_create(copy_constructor_type copy_constructor,
    ...
    this -> array = (void**) malloc(INITIAL_CAPACITY * sizeof(default_constructor_type));
    this -> size = 0;
...
void vector_resize(vector *this, size_t n) {
    ...
    else if(n <= this -> capacity && n > this -> size)
    {
...
void vector_insert(vector *this, size_t position, void *element) {
    ...
    for (size_t i = this -> size-1; i >= position && i <= this -> size; i--)
    {
