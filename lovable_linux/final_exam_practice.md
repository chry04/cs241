...
1.	What are the differences between a library call and a system call? Include an example of each.
       library is provided by c libraries and system call is provided by kernal. printf vs write

2.	What is the `*` operator in C? What is the `&` operator? Give an example of each.
        *means a pointer, *p means to read the value in this address char* p.
        & means the address, it will pass the address of a variable. &p
...
4.	How are C strings represented in memory? What is the wrong with `malloc(strlen(s))` when copying strings?
       an array of char end with '\0'. malloc(strlen(s)) will not copy the null byte.
